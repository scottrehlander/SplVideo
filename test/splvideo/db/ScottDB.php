<?php
	...
	class DbAccess
	{
		...
		function createKeywordSelect($keywords, $isOnlyCount)
		{
			if($isOnlyCount)
				$sql = "SELECT count(k.videoId) FROM keywords k inner join videos v on v.videoId = k.videoId where ";
			else
		...
		function getNumberOfVideosFromKeywords($keywords)
		{
			//This accepts a string of space separated keywords
			$keywords = explode(" ", $keywords);

			$vars = array();
			$sql = $this->createKeywordSelect($keywords, false);
			$result = $this->ExecutePreparedQuery($sql, $array);
			return count($result);
		}
		
		function getVideosFromKeywords($keywords, $sortString, $startIndex, $numOfVideos)
		{
			//This accepts a string of space separated keywords
			$keywords = explode(" ", $keywords);
			...
			if(isset($numOfVideos))
				$sql .= " LIMIT " . $startIndex . ', ' . $numOfVideos;
